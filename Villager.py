...
class Villager_sleeping(State):
    ...
    def check_conditions(self):
        ...
        if (self.Villager.world.clock_degree < 190 or self.Villager.world.clock_degree > 250) and (self.Villager.world.wood < self.Villager.world.MAXwood):
            return "exploring"
...
class Villager_Exploring(State):
    ...
    def do_actions(self):
        if self.Villager.location.get_distance_to(self.Villager.destination) < 2.0:    #This is because it will always be < 1 away, but never get it exactly
            self.random_dest()

    def check_conditions(self):

        if (self.Villager.world.clock_degree < 250. and self.Villager.world.clock_degree >190.) or (self.Villager.world.wood >= self.Villager.world.MAXwood):
            return "sleeping"
